...
## How I'd run a team on this

Three folder owners:
- **AI engineer** owns `app/services/`, `app/prompts/`, `scripts/`. Models, prompts, retrieval, eval.
- **Platform engineer** owns `app/core/`, `app/observability/`, `Dockerfile`, deployment. Vector store, secrets, infra.
- **Forward-deployed engineer** per customer integration: owns `app/api/` extensions, customer-specific eval cases, document-format support.

The split exists because these are three different jobs and the people who are good at them are rarely the same people. The folder boundaries make that explicit. The interfaces between folders (the `DocumentStore` ABC, the `get_llm` factory, the prompt registry, the pydantic schemas) are the contract.

What I'd hire for first if this were a real team: the eval engineer. Without good eval, every other improvement is faith-based.
